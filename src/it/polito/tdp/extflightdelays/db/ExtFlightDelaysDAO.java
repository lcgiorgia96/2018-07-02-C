...
public class ExtFlightDelaysDAO {
	...
	public List<Airport> getAer(int x) {
		String sql ="SELECT A1.ID, A1.IATA_CODE,A1.AIRPORT,A1.CITY,A1.STATE,A1.COUNTRY,A1.LATITUDE,A1.LONGITUDE,A1.TIMEZONE_OFFSET " + 
				"FROM airports AS A1, flights AS F1 " + 
				"WHERE f1.DESTINATION_AIRPORT_ID = A1.ID OR f1.ORIGIN_AIRPORT_ID=A1.ID " + 
				"GROUP BY A1.ID " + 
		...
		try {
			...
			while (rs.next()) {
				if (idMap.get(rs.getInt("ID1"))!=null && idMap.get(rs.getInt("ID2"))!=null) {
					res.add(new Rotta(idMap.get(rs.getInt("ID1")),idMap.get(rs.getInt("ID2")),rs.getInt("VOLI")));
				}
			}
